...
def read_in_all_images_100(address,shuffle=True, is_random_label = False):
    """
    This function reads all training or validation data, shuffles them if needed, and returns the
    images and the corresponding labels as numpy arrays
    ...
    # Concatenate along axis 0 by default 数组拼接
    #data = np.concatenate((data, batch_data))
    #label = np.concatenate((label, batch_label))
...
def prepare_train_data(padding_size):
    ...
    if FLAGS.datasets_num == 10:
        ...
        data, label = read_in_all_images_100(full_data_dir, is_random_label=TRAIN_RANDOM_LABEL)
...
def read_validation_data():
    ...
    if FLAGS.datasets_num == 10:
        ...
        validation_array, validation_labels = read_in_all_images_100(vali_dir,
                                                       is_random_label=VALI_RANDOM_LABEL)
