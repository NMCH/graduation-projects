...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        loss = self.loss(predictions, self.label_placeholder)
        self.full_loss = tf.add_n([loss] + regu_losses)
    ...
    def loss(self, logits, labels):
        ...
        y_=tf.one_hot(labels,FLAGS.datasets_num,on_value=1,off_value=0,axis=1)
        y=logits
        cross_entropy=-tf.reduce_mean(y_*tf.log(tf.clip_by_value(y,1e-10,1.0)))
        #均方根
        #labels=tf.one_hot(labels,FLAGS.datasets_num,on_value=1,off_value=0,axis=1)
        #mean_squared_error=tf.losses.mean_squared_error(                 #均方根loss
        #    labels=labels,
        #    predictions=logits,
        #    weights=1.0,
        #    scope=None
        # )
        #MSE_mean=tf.reduce_mean(mean_squared_error,name='mean_squared_error')

        #EMD loss
        #EMD=emd.emd_loss(labels, logits, r=2)
        
        #focal loss
        #labels = tf.cast(labels, tf.float32)
        #focalLoss=focal_loss.focal_loss(logits,labels)
        #加权交叉熵
        #weighted_cross_entropy = tf.nn.weighted_cross_entropy_with_logits(    #权重交叉熵loss
        #   targets=labels,
        #   logits=logits,
        #    pos_weight=2
        #)tf.reduce_mean(weighted_cross_entropy)
